...
app.post("/user", async (req, res) => {
  const q =
    "INSERT INTO user (`usernom`, `userprenom`, `userphone`) VALUES (?,?,?)";
  const values = [req.body.usernom, req.body.userprenom, req.body.userphone];
...
app.put("/user/:id", async (req, res) => {
  ...
  const q =
    "UPDATE user SET usernom = ?, userprenom = ?, userphone = ? WHERE userid = ?";
  const values = [req.body.usernom, req.body.userprenom, req.body.userphone];
...
app.get("/user/:id", (req, res) => {
  const userid = req.params.id;
  const q =
    "SELECT  `userid`, `usernom`, `userprenom`, `userphone` FROM user WHERE userid = ?";
  db.query(q, [userid], (err, data) => {
    if (err) return res.json(err);
    return res.json(data);
  });
